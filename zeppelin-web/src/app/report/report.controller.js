...
angular.module('zeppelinWebApp').controller('ReportCtrl', ReportCtrl);

ReportCtrl.$inject = ['$scope', '$rootScope', '$http', 'baseUrlSrv', 'ngToast'];

function ReportCtrl($scope, $rootScope, $http, baseUrlSrv, ngToast) {
  $scope._ = _;
  ngToast.dismiss();
  ...
  var getCredentialInfo = function() {
    $http.get(baseUrlSrv.getRestApiBase() + '/report').
  ...
  $scope.addNewCredentialInfo = function() {
    ...
    $http.put(baseUrlSrv.getRestApiBase() + '/report', newCredential).
    success(function(data, status, headers, config) {
      ngToast.success({
  ...
  $scope.updateCredentialInfo = function(form, data, entity) {
    ...
    $http.put(baseUrlSrv.getRestApiBase() + '/report/', request).
    success(function(data, status, headers, config) {
  ...
  $scope.removeCredentialInfo = function(entity) {
    BootstrapDialog.confirm({
      ...
      callback: function(result) {
        if (result) {
          $http.delete(baseUrlSrv.getRestApiBase() + '/report/' + entity).
          success(function(data, status, headers, config) {
            var index = _.findIndex($scope.credentialInfo, {'entity': entity});
