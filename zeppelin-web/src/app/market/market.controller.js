...
angular.module('zeppelinWebApp').controller('MarketCtrl', MarketCtrl);

MarketCtrl.$inject = ['$scope', '$rootScope', '$http', 'baseUrlSrv', 'ngToast'];

function MarketCtrl($scope, $rootScope, $http, baseUrlSrv, ngToast) {
  $scope._ = _;
  ngToast.dismiss();
  ...
  var getCredentialInfo = function() {
    $http.get(baseUrlSrv.getRestApiBase() + '/market').
  ...
  $scope.addNewCredentialInfo = function() {
    ...
    $http.put(baseUrlSrv.getRestApiBase() + '/market', newCredential).
    success(function(data, status, headers, config) {
      ngToast.success({
  ...
  $scope.updateCredentialInfo = function(form, data, entity) {
    ...
    $http.put(baseUrlSrv.getRestApiBase() + '/market/', request).
    success(function(data, status, headers, config) {
  ...
  $scope.removeCredentialInfo = function(entity) {
    BootstrapDialog.confirm({
      ...
      callback: function(result) {
        if (result) {
          $http.delete(baseUrlSrv.getRestApiBase() + '/market/' + entity).
          success(function(data, status, headers, config) {
            var index = _.findIndex($scope.credentialInfo, {'entity': entity});
