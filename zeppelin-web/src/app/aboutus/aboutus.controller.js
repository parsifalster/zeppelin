...
angular.module('zeppelinWebApp').controller('AboutusCtrl', AboutusCtrl);

AboutusCtrl.$inject = ['$scope', '$rootScope', '$http', 'baseUrlSrv', 'ngToast'];

function AboutusCtrl($scope, $rootScope, $http, baseUrlSrv, ngToast) {
  $scope._ = _;
  ngToast.dismiss();
  ...
  var getCredentialInfo = function() {
    $http.get(baseUrlSrv.getRestApiBase() + '/aboutus').
  ...
  $scope.addNewCredentialInfo = function() {
    ...
    $http.put(baseUrlSrv.getRestApiBase() + '/aboutus', newCredential).
    success(function(data, status, headers, config) {
      ngToast.success({
  ...
  $scope.updateCredentialInfo = function(form, data, entity) {
    ...
    $http.put(baseUrlSrv.getRestApiBase() + '/abortus/', request).
    success(function(data, status, headers, config) {
  ...
  $scope.removeCredentialInfo = function(entity) {
    BootstrapDialog.confirm({
      ...
      callback: function(result) {
        if (result) {
          $http.delete(baseUrlSrv.getRestApiBase() + '/abortus/' + entity).
          success(function(data, status, headers, config) {
            var index = _.findIndex($scope.credentialInfo, {'entity': entity});
